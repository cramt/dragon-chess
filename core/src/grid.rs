    ...
    T: Default + PartialEq,
{
    ...
    pub fn flat_with_index_owned(&self) -> Vec<(Vector3, T)>
    where
        T: Copy,
    {
        let mut v = vec![];
    ...
    pub fn concat(&mut self, other: Grid<T>)
    where
        T: Copy,
    {
        for x in 0..self.array.len() {
...
impl<T> Index<&'_ Vector3> for Grid<T>
where
    T: Default + PartialEq,
{
    type Output = T;
...
impl<T> IndexMut<&'_ Vector3> for Grid<T>
where
    T: Default + PartialEq,
{
